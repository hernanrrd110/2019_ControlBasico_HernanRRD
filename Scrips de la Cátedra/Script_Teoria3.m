...
%%
%La respuesta a un escalón en un sistema de segundo orden para zita>1 es y4
% y también puede verse en la transparencia de teoría:

clear zi;  %Para que no tome zi=0 como se le asignó la última vez
syms zi
p1=-wn*(zi+sqrt(zi^2-1));
p2=-wn*(zi-sqrt(zi^2-1));
y4=simplify(K*M*(1-(wn/(2*sqrt(zi^2-1)))*(exp(p1*t)/p1-exp(p2*t)/p2)))

%Para obtener la respuesta al impulso derivo la expresión anterior: 
...
%Comparación de la respuesta al escalón y la delta para un mismo sistema
%con zita=0.4
zi=0.4
